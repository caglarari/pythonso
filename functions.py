...
_libfunctions = numpy.ctypeslib.load_library('libfuncs', '.')
...
_libfunctions.showtime.argtypes=[ctypes.c_int]
_libfunctions.showtime.restype=ctypes.c_void_p

def hello(n):
    return _libfunctions.hello(int(n))

def showtime(n):
    return _libfunctions.showtime(int(n))
